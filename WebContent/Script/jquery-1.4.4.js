//设置一个Cookie
function setCookie(cookie_temppara, value) {
    var exdate = new Date()
...
/*获取Url后面参数*/
/*paras为参数名称*/
function GetUrlParaValue(paras) {
...
//检测一个文本框是否为空并给出提示
var GetTextValue = function(id, _alert) {
    if ($(id).val() == "") {
        alert(_alert);
        return false;
    }
...
//日期格式化  参数为字符串型
function DataFormat(strDate, obj_format) {
    //先转为日期格式
    var datef = new Date(Date.parse(strDate.replace(/-/g, "/")));
    return datef.format(obj_format);
}
//检测日期格式是否正确
